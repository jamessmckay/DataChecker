...
export class HomeComponent implements OnInit {
  ...
  environmentTypes = [];
  listTags: Tag[] = [];
  existChangesInRuleSearch = false;
  existChangesInDownloadCommunityCollection = false;
  ...
  rulesFilter: RuleFilter = new RuleFilter();
  ...
  ngOnInit() {
    ...
    this.apiService.databaseEnvironment
      .getDatabaseEnvironments()
      .subscribe((databaseEnvironments) => {
    ...
    this.apiService.container.getAllCollections().subscribe((collections) => {
      if (collections != null) {
        collections.forEach(
          (rec: { isDefault: boolean }) => (rec.isDefault = false)
        );
  ...
  addCollection(content: any) {
    ...
    this.addCollectionObjModal = this.modalService.open(content, {
      ...
      () => {
        ...
        this.apiService.container
          ...
          .subscribe((result) => {
            ...
            this.toastr.success(
              'Collection ' + this.newCollection.name + ' Added',
              'Success'
            );
          });
      },
  ...
  changeCollection(category: Category) {
    ...
    if (this.selectedDatabaseEnvironment !== undefined) {
      this.apiService.container
        .getContainerByContainerIdAndDatabaseEnvironmentId(
          category.id,
  ...
  executeAll() {
    this.apiService.databaseEnvironment
      .testDatabaseEnvironmentById(this.selectedDatabaseEnvironment)
      .subscribe((isConnectedMessage) => {
        if (isConnectedMessage == null || isConnectedMessage === '') {
          ...
          this.toastr.error(
            'Review the connection with your environment:' + isConnectedMessage,
          ...
          if (isConnectedMessage == null || isConnectedMessage === '') {
            ...
            category.rules.forEach((rule) => {
              ...
              this.apiService.rule
                .executeRule({
                  ruleId: rule.id,
                  databaseEnvironmentId: this.selectedDatabaseEnvironment.id,
                })
                .subscribe(
                  (result) => {
            ...
            this.toastr.error(
              'Review the connection with your environment:' +
                isConnectedMessage,
              'Environment Connection Error'
            );
          }
  ...
  setContainerStatus(category: Category) {
    if (
      ...
      category.validRules + category.invalidRules + category.sintaxRules
    ) {
      ...
      const remainingCategories = this.categories.filter(
        (rec) => rec.rules.length > 0 && rec.lastStatus === 0
      );
  ...
  deleteCollectionFromChild(element: { isDefault: any }) {
    this.apiService.container.getAllCollections().subscribe((collections) => {
      this.collections = collections;
      if (element.isDefault && this.collections.length > 0) {
        ...
        const existCategoryDefault = this.collections.find(
          (rec) => rec.isDefault
        );
        if (
          existCategoryDefault != null &&
          existCategoryDefault !== undefined
        ) {
          this.collections.forEach((m) => (m.isDefault = false));
  ...
  updateRuleResultFromChild(ruleFromChild: Rule) {
    const container = this.selectedCategory.childContainers.find(
      (rec: { id: string; }) => rec.id === ruleFromChild.containerId
    );
    if (container != null) {
      const rule = container.rules.find((rec: { id: string; }) => rec.id === ruleFromChild.id);
      if (rule != null) {
        ...
        const rulesInvalid = container.rules.filter(
          (rec: { lastStatus: number; }) => rec.lastStatus === 2
        );
        const rulesError = container.rules.filter(
          (rec: { lastStatus: number; }) => rec.lastStatus !== 1 && rec.lastStatus !== 2
        );
        if (rulesError.length > 0) {
          container.lastStatus = 0;
        }
        // 0 is when not all rules have been ran
        else if (rulesInvalid.length > 0) {
          container.lastStatus = 2;
        }
        // 2 is when exist at least one error
        else {
          container.lastStatus = 1;
        } // 1 is when all rules ran properly
  ...
  reloadDataFromModal() {
    if (
      this.existChangesInRuleSearch ||
      this.existChangesInDownloadCommunityCollection
    ) {
      this.apiService.container.getAllCollections().subscribe((collections) => {
        this.collections = [];
        if (collections != null) {
  ...
  showSignInToRepository(signInContent: any, downloadCommunityContent: any) {
  ...
  signInToRepository(downloadCommunityContent: any) {
  ...
  showCommunityCollections(downloadCommunityContent: any) {
    this.existChangesInDownloadCommunityCollection = false;

    this.apiService.community
      .getCommunityCollections(this.tokenInformation)
      .subscribe((result) => {
          ...
          result.forEach(
            (rec: {
              ...
              containerDestination:
                | { destinationStructure: string | null | undefined }
                | null
                | undefined;
              containerDestinationStructure: any;
            }) => {
              rec.showDestinationStructure = false;
              if (
                rec.containerDestination != null &&
                rec.containerDestination !== undefined &&
                rec.containerDestination.destinationStructure != null &&
                rec.containerDestination.destinationStructure !== undefined
              ) {
                rec.containerDestinationStructure = JSON.parse(
                  rec.containerDestination.destinationStructure
                );
              }
            }
          );
  ...
  filterRulesOfCategories() {
    ...
    for (let i = 0; i < this.categories.length; i++) {
      ...
      if (
        this.categories[i].rules !== undefined &&
        this.categories[i].rules != null &&
        this.categories[i].rules.length > 0
      ) {
        // tslint:disable-next-line: prefer-for-of
        for (let j = 0; j < this.categories[i].rules.length; j++) {
          if (
            this.rulesFilter.rulesSuccess &&
            this.categories[i].rules[j].lastStatus === 1
          ) {
            rules.push(this.categories[i].rules[j]);
          }

          if (
            this.rulesFilter.rulesValidation &&
            this.categories[i].rules[j].lastStatus === 2
          ) {
            rules.push(this.categories[i].rules[j]);
          }

          if (
            this.rulesFilter.rulesError &&
            this.categories[i].rules[j].lastStatus === 3
          ) {
            rules.push(this.categories[i].rules[j]);
          }

          if (
            this.rulesFilter.rulesNotRan &&
            this.categories[i].rules[j].lastStatus === 0
          ) {
            rules.push(this.categories[i].rules[j]);
          }
        }
        ...
        if (rules.length > 0) {
          ...
          newContainer.validRules = rules.filter(
            (rec) => rec.lastStatus === 2
          ).length;
