...
export class JobsComponent implements OnInit {
  ...
  getDatabaseEnvironmentName(job: Job) {
    const dbe = (this.databaseEnvironments || []).find(
      (d) => d.id === job.databaseEnvironmentId
    );
    if (dbe) {
      return dbe.name;
    }
  ...
  getTargetName(job: Job) {
    ...
    if (job.type === 1) {
      const tag = (this.tags || []).find((d) => d.id === job.tagId);
      if (tag) {
        return tag.name;
      }
    }
  ...
  saveJob(jobForm: FormGroup, jobMessageModal: any) {
    if (!jobForm.valid) {
    ...
    if (this.newJob.type === 1) {
      this.newJob.containerId = '';
    }
    if (this.newJob.type === 2) {
      this.newJob.tagId = -1;
    }
    ...
    if (this.jobs != null && this.jobs.length > 0) {
      existJobSameCron = this.jobs.find(
        (rec) => rec.cron === this.newJob.cron && rec.id !== this.newJob.id
      );
    }
  ...
  showCronInfo() {
    window.open('https://crontab.guru/', '_blank', undefined, undefined).focus();
  }
