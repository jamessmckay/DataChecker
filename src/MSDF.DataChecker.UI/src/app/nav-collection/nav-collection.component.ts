...
export class NavCollectionComponent implements OnInit {
  @Input()
  selectedDatabaseEnvironment: DatabaseEnvironment = new DatabaseEnvironment();
  ...
  ruleCategoryChild: Category = new Category();
  ...
  moveRuleModal: any;
  containerIdSelectedToMove!: any;
  ruleToMove!: any;
  containerSelectedToMove!: any;
  ...
  ngOnInit() {
    ...
    if (this.category.childContainers != null) {
      this.category.childContainers.forEach(
        (rec) => (this.category.amountRules += rec.amountRules)
      );
    }
    ...
    this.apiService.catalog
      .getByType('RuleDetailsDestinationType')
      .subscribe((result) => {
  ...
  onAddingTag(tag: any) {
    const existTag = this.newCollection.tags.find((rec) => rec.id === tag.id);
    if (existTag == null) {
  ...
  onRemovingTag(tag: any) {
    this.newCollection.tags = this.newCollection.tags.filter(
      (rec) => rec.id !== tag.id
    );
  }

  updateCollection(content: any) {
    this.loadTags();
    ...
    if (
      this.newCollection.ruleDetailsDestinationId === undefined ||
        ...
        (result) => {
          if (this.newCollection.containerTypeId === 1) {
            if (
              ...
              this.newCollection.ruleDetailsDestinationId = parseInt(
                this.newCollection.ruleDetailsDestinationId.toString()
              );
            } else {
              this.newCollection.ruleDetailsDestinationId = -1;
            }
          }

          this.newCollection.environmentType = parseInt(
            this.newCollection.environmentType.toString()
          );
          this.apiService.container
            ...
            .subscribe((localUser) => {
              ...
              if (this.category.isDefault) {
                this.selectCategoryFromChild.emit(this.category);
              }
              this.toastr.success(
                'Collection ' + this.newCollection.name + ' updated',
                'Success'
              );
            });
        },
  ...
  setDeleteCollectionMessage() {
    ...
    if (
      this.category.amountRules > 0 ||
      (this.category.childContainers !== undefined &&
        this.category.childContainers.length > 0)
    ) {
      const totalContainers = this.category.childContainers.length;
      let totalRules = 0;

      this.category.childContainers.forEach(
        (rec) => (totalRules += rec.rules.length)
      );
      this.deleteCollectionContainerMessage =
      ...
      if (totalRules > 0) {
        this.deleteCollectionContainerMessage +=
          ' and ' + totalRules + ' ' + (totalRules === 1 ? 'rule' : 'rules');
      }
      this.deleteCollectionContainerMessage += '?';
    }
  }
  ...
  setDeleteContainerMessage(categoryChild: Category) {
    this.deleteCollectionContainerMessage = '';
    if (
      categoryChild.amountRules > 0 ||
      categoryChild.rules !== undefined ||
      categoryChild.rules != null
    ) {
      const totalRules = categoryChild.rules.length;
      if (totalRules > 0) {
        this.deleteCollectionContainerMessage +=
          'Are you sure to delete ' +
          totalRules +
  ...
  deleteCollection() {
    this.apiService.container
      .deleteContainer(this.category.id)
      .subscribe((rec) => {
        if (this.category.isDefault) {
          this.deleteCollectionFromChild.emit(this.category);
        } else {
          const index = this.collections.indexOf(this.category);
          this.category.childContainers.splice(index, 1);
        }
        this.toastr.success(
          'Container ' + this.category.name + ' deleted',
          'Success'
        );
        this.deleteCollectionFromChild.emit(this.category);
      });
        ...
        () => {
          this.apiService.container
            ...
            .subscribe((addedContainer) => {
              this.category.childContainers.push(
                Object.assign({}, addedContainer)
              );
              if (this.category.isDefault) {
                this.selectCategoryFromChild.emit(this.category);
              }
              this.toastr.success(
                'Container ' + addedContainer.name + ' added',
                'Success'
              );
            });
        },
  ...
  updateContainer(content: any, categoryChild: any) {
    this.loadTags();
  ...
  deleteContainer(categoryChild: Category) {
    this.apiService.container
      .deleteContainer(categoryChild.id)
      .subscribe(() => {
        const index = this.category.childContainers.indexOf(categoryChild);
        this.category.childContainers.splice(index, 1);
        ...
        this.toastr.success(
          'Container ' + categoryChild.name + ' deleted',
          'Success'
        );
      });
  }
  ...
  setDefault() {
    this.apiService.container
      .setDefaultContainer(this.category)
      .subscribe((rec) => {
        this.toastr.success(
          'Collection ' + this.category.name + ' set to default',
          'Success'
        );
      });
  }
  ...
  setEnvironmentTypeFromCollection() {
    if (
      this.category.environmentType !== undefined &&
      this.category.environmentType != null &&
      this.category.environmentType > 0
    ) {
      const existEnvironment = this.environmentTypes.find(
        (rec) => rec.id === this.category.environmentType
      );
      if (existEnvironment) {
  ...
  validateSqlDiagnostic(): boolean {
    let result = true;
    if (
      this.newRule.diagnosticSql !== undefined &&
      this.newRule.diagnosticSql != null &&
      this.newRule.diagnosticSql !== ''
    ) {
      result = false;
      const sqlDiagnostic = this.newRule.diagnosticSql.toLowerCase();
      if (sqlDiagnostic.indexOf('select') === 0) {
        const sqlSplit = sqlDiagnostic.split('from');
        ...
        if (sqlSplit.length > 1) {
          if (sqlSplit[0].indexOf('educationorganizationid') > 0) {
            ...
            this.toastr.error(
              'The Diagnostic Sql should have the column EducationOrganizationId',
              'Error Diagnostic Sql'
            );
          }
        } else {
          this.toastr.error(
            'The Diagnostic Sql should have the FROM statement',
            'Error Diagnostic Sql'
          );
        }
      } else {
        this.toastr.error(
          'The Diagnostic Sql should start with the SELECT statement',
          'Error Diagnostic Sql'
        );
      }
    }
  ...
  addRule(ruleContent: any, categoryChild: Category) {
    this.loadTags();
    this.runSqlTest = false;

    if (
      this.selectedDatabaseEnvironment == null ||
      this.selectedDatabaseEnvironment === undefined
    ) {
    ...
    this.apiService.databaseEnvironment
      .getDatabaseInfo(this.selectedDatabaseEnvironment)
      .subscribe((result) => {
  ...
  modifyRule(ruleContent: any, ruleChild: Rule, categoryChild: Category) {
    this.loadTags();
    this.runSqlTest = false;
    ...
    this.apiService.ruleExecutionLogDetail
      .getLastRuleExecutionLogByEnvironmentAndRuleAsync(
        this.selectedDatabaseEnvironment.id,
        this.newRule.id
      )
      .subscribe((result) => {
  ...
  validateIncreaseVersion(increaseVersionNumberMessageContent: any) {
    if (
      this.newRule.id === '00000000-0000-0000-0000-000000000000' ||
      this.newRule.version !== this.oldRule.version
    ) {
      this.saveRule();
      return;
    }

    this.ruleIncreaseVersionModal = this.modalService.open(
      increaseVersionNumberMessageContent,
      {
        ariaLabelledBy: 'modal-basic-title',
        backdrop: 'static',
        centered: true,
  ...
  saveRule() {
    ...
    this.newRule.errorSeverityLevel = parseInt(
      this.newRule.errorSeverityLevel.toString()
    );

    if (
      this.newRule.maxNumberResults !== undefined &&
      this.newRule.maxNumberResults !== null
    ) {
      this.newRule.maxNumberResults = parseInt(
        this.newRule.maxNumberResults.toString()
      );
    }

    if (
      this.newRule.tags !== undefined &&
      this.newRule.tags != null &&
      this.newRule.tags.length > 0
    ) {
    ...
    if (this.modalRuleDescription === 'Add Sql Data Check') {
      this.apiService.rule.addRule(this.newRule).subscribe((data) => {
        ...
        if (this.ruleWarningModal != null) { this.ruleWarningModal.close(); }
        ...
        this.toastr.success(
          'Sql Data Check ' + this.newRule.name + ' added ',
          'Success'
        );
        if (newTags > 0) {
          this.loadTags();
          this.toastr.success(
            'You have created ' + newTags + ' new tags.',
            'Success'
          );
        }
      });
      ...
      this.apiService.rule.modifyRule(this.newRule).subscribe((data) => {
        ...
        this.ruleCategoryChild.rules.forEach((rec, index) => {
          if (rec.id === this.newRule.id) {
            this.ruleCategoryChild.rules[index] = Object.assign(
              {},
              this.newRule
            );
          }
        ...
        this.toastr.success(
          'Rule ' + this.newRule.name + ' modified ',
          'Success'
        );
        if (newTags > 0) {
          this.loadTags();
  ...
  cancelRule(forceClose: any, ruleWarningContent: any) {
    if (forceClose) {
      if (this.ruleWarningModal != null) { this.ruleWarningModal.close(); }
      this.ruleModal.close();
    } else {
      if (this.oldRule != null) {
        ...
        const existDifferences = UtilService.compareObj(
          this.newRule,
          this.oldRule
        );
        for (const key in existDifferences) {
          if (
            key !== 'environmentTypeText' &&
            key !== 'id' &&
            key !== 'idLastRuleExecutionLog'
          ) {
            countDifferences++;
          }
        }
  ...
  onAddingRuleTag(tag: any) {
    const existTag = this.newRule.tags.find((rec) => rec.id === tag.id);
    if (existTag == null) {
  ...
  deleteRuleChild(ruleChild: Rule, categoryChild: Category) {
    this.apiService.rule.deleteRule(ruleChild.id).subscribe((result) => {
      const index = categoryChild.rules.indexOf(ruleChild);
      ...
      this.toastr.success(
        'Sql Data Check ' + ruleChild.name + ' deleted',
        'Success'
      );
    });
  }
  ...
  runSqlRule(runType: string) {
    ...
    if (runType === 'sql') {
      ...
      this.apiService.rule
        .executeRuleTest(
          { diagnosticSql: this.newRule.diagnosticSql },
          this.selectedDatabaseEnvironment.id
        )
        .subscribe((result) => {
          if (result.result >= 0) {
  ...
  copyRule(ruleChild: Rule, categoryChild: Category) {
    this.apiService.rule
      ...
      .subscribe((result) => {
        ...
        this.toastr.success(
          'Sql Data Check ' + ruleChild.name + ' copied',
          'Success'
        );
      });
  }
  ...
  copyRuleSelected(ruleCopyRulesToContent: any) {
  ...
  deleteRuleSelected() {
    this.apiService.rule.deleteRule(this.newRule.id).subscribe((result) => {
      const index = this.ruleCategoryChild.rules.indexOf(this.newRule);
      ...
      this.toastr.success(
        'Sql Data Check ' + this.newRule.name + ' deleted',
        'Success'
      );
      this.ruleModal.close();
    });
  ...
  allContainersSelectedChange(event: any) {
    if (this.listChildContainers.length > 0) {
      this.listChildContainers.forEach(
        (rec) => (rec.selected = this.allContainersSelected)
      );
    }
  }

  saveCopyRulesTo() {
    const containersSelected = this.listChildContainers.filter(
      (rec) => rec.selected
    );
    ...
    const rulesId = [];
    const containersId: string[] = [];
    rulesId.push(this.newRule.id);
    containersSelected.forEach((rec) => containersId.push(rec.id));
    this.apiService.rule
      .copyRulesToContainers({ ruleIds: rulesId, containerIds: containersId })
      .subscribe(() => {
  ...
  viewRuleExecutionLogDetailsFromOptions(
    ruleExecutionLogDetailRuleEditContent: any,
    ruleChild: Rule
  ) {
    if (
      this.selectedDatabaseEnvironment == null ||
    ...
    this.apiService.ruleExecutionLogDetail
      ...
      .subscribe((idResult) => {
        ...
        if (idExecutionLog > 0) {
          this.apiService.ruleExecutionLogDetail
            .getByRuleExecutionLogAsync(idExecutionLog)
            .subscribe((result) => {
              if (result != null) {
                ...
                this.toastr.info(
                  'No execution log details for rule ' + ruleChild.name,
                  'Information'
                );
              }
            });
  ...
  viewRuleExecutionLogDetails(ruleExecutionLogDetailRuleEditContent: any) {
    this.ruleDetailsLogResult = null;
    const idExecutionLog = this.newRule.idLastRuleExecutionLog;
    this.apiService.ruleExecutionLogDetail
      .getByRuleExecutionLogAsync(idExecutionLog)
      .subscribe((result) => {
        this.ruleDetailsLogResult = result;
  ...
  exportToCsv() {
    if (
      this.ruleDetailsLogResult != null &&
      this.ruleDetailsLogResult !== undefined &&
      this.ruleDetailsLogResult.ruleExecutionLogId > 0
    ) {
      const url = this.apiService.ruleExecutionLogDetail.exportToCsvByRuleExecutionLogAsync(
        this.ruleDetailsLogResult.ruleExecutionLogId
      );
  ...
  exportToTable() {
    if (
      ...
      this.apiService.ruleExecutionLogDetail
        .exportToTableByRuleExecutionLogAsync(
          this.ruleDetailsLogResult.ruleExecutionLogId
        )
        .subscribe((result) => {
          if (result.alreadyExist) {
            this.toastr.success(
              'Rule execution log details already exported to table ' +
                result.tableName,
              'Export to Table'
            );
          } else if (result.created) {
            this.toastr.success(
              'Rule execution log details exported to table ' +
                result.tableName,
              'Export to Table'
            );
          }
  ...
  showSignInToRepository(
    signInContent: any,
    uploadCollectionMessageContent: any
  ) {
    ...
    if (this.tokenInformation == null) {
      ...
      this.apiService.community.isTokenValid(this.tokenInformation).subscribe(
        (result) => {
  ...
  signInToRepository(uploadCollectionMessageContent: any) {
    UtilService.communityUrlUser = this.userSignIn.server;
    this.apiService.community.signInToCommunity(this.userSignIn).subscribe(
      (result) => {
      ...
      (error) => {
        if (error.status === 401) {
          ...
          this.toastr.error(
            'Please verify you have connection with the Community server.',
            'Sign In Error'
          );
        }
      }
  ...
  uploadCollection(uploadCollectionMessageContent: any) {
    this.apiService.community
      .getCollectionByName(this.category, this.tokenInformation)
      .subscribe((result) => {
        if (result == null) {
          ...
          this.uploadCollectionMessageModal = this.modalService.open(
            uploadCollectionMessageContent,
            {
              ariaLabelledBy: 'modal-basic-title',
              backdrop: 'static',
            }
          ...
          this.toastr.error(
            'You cannot upload the selected collection because exist one collection in the community with the same name and you cannot overwrite it.',
            'Upload Collection'
          );
        }
      });
  ...
  continueUploadCollection() {
    this.apiService.container
      .getContainerToCommunity(this.category.id)
      .subscribe((result) => {
        this.apiService.community
          .validateDestinationTable(
            result.containerDestination,
            this.tokenInformation
          )
          .subscribe((resultValidation) => {
            if (resultValidation) {
              this.apiService.community
                .uploadCollection(result, this.tokenInformation)
                .subscribe(() => {
                  if (this.signInModal != null) {
                    this.signInModal.close();
                    this.signInModal = null;
              ...
              this.toastr.error(
                'You cannot upload a collection if the DestinationTable does not have the same structure.',
                'Upload Collection Failed'
              );
            }
          });
      });
  ...
  onContainerToMoveItemChange(item: Category) {
    this.containerIdSelectedToMove = item;
  }

  saveMoveRuleTo() {
    if (
      this.containerIdSelectedToMove === undefined ||
      this.containerIdSelectedToMove == null
    ) {
    ...
    this.apiService.rule
      .moveRuleToContainer(rulesId, this.containerIdSelectedToMove)
      .subscribe((result) => {
        this.toastr.success(
          'Rule moved to container successfully.',
          'Move Rule'
        );
        this.reloadDataFromChild.emit(true);
