...
@Component({
  selector: 'app-nav-community-collection',
...
export class NavCommunityCollectionComponent implements OnInit {
  ...
  validDownloadCollection(downloadCollectionMessageContent: any) {
    this.apiService.container
      .getContainerByName(this.category)
      .subscribe((result) => {
        if (result == null) {
          this.downloadCollection(true);
        } else {
          this.downloadCollectionMessageModal = this.modalService.open(
            downloadCollectionMessageContent,
            {
  ...
  downloadCollection(newCollection: boolean) {
    this.category.createNewCollection = newCollection;
    this.apiService.container
      .validateDestinationTable(this.category.containerDestination)
      .subscribe((resultValidation) => {
        if (resultValidation) {
          this.apiService.container
            .addContainerFromCommunity(this.category)
            .subscribe((result) => {
          ...
          this.toastr.error(
            'You cannot download a collection if the DestinationTable does not have the same structure.',
  ...
  showRule(rule: any, ruleCommunityContent: any) {
    this.newRule = rule;
    this.newRule.diagnosticSql = sqlFormatter.format(
