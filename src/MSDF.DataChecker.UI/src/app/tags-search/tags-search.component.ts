...
export class TagsSearchComponent implements OnInit {
  ...
  dropdownSettings = {};
  collectionList: any[] = [];
  selectedCollectionItems: any[] = [];
  containerList: any[] = [];
  selectedContainerItems: any[] = [];
  tagList: any[] = [];
  ...
  ngOnInit() {
    ...
    this.apiService.container.getParentContainers().subscribe((result) => {
      const tmp: any[] = [];
      result.forEach((rec: { id: any; name: any }) =>
        tmp.push({
          item_id: rec.id,
          item_text: rec.name,
        })
    ...
    this.apiService.tag.getTags().subscribe((result) => {
      ...
      const tmp: { item_id: any; item_text: any }[] = [];
      result.forEach((rec: { id: any; name: any }) =>
        tmp.push({
          item_id: rec.id,
          item_text: rec.name,
  ...
  search() {
    const tmpCollections: any[] = [];
    const tmpContainers: any[] = [];
    const tmpTags: any[] = [];

    this.selectedCollectionItems.forEach((rec) =>
      tmpCollections.push(rec.item_id)
    );
    this.selectedContainerItems.forEach((rec) =>
      tmpContainers.push(rec.item_id)
    );
    ...
    const searchRequest = {
      ...
      detailsDestination: -1,
      severity: -1,
    };

    if (this.searchModel.ruleDestination !== '0') {
      searchRequest.detailsDestination = parseInt(
        this.searchModel.ruleDestination
      );
    }
    ...
    this.apiService.rule.searchRules(searchRequest).subscribe((result) => {
      ...
      this.isDeleteDisabled = !(
        this.rules.length > 0 &&
        this.rules.filter((rec) => rec.selected).length > 0
      );
    });
  }
  ...
  modifyRule(ruleContent: any, ruleChild: Rule) {
    this.loadCatalogs();
    this.loadTags();
  ...
  showCopyRulesSelected(ruleCopyRulesToContent: any) {
  ...
  saveCopyRulesTo() {
    if (this.ruleModal != null) {
      this.apiService.rule
        .copyRulesToContainers({
          ruleIds: [this.newRule.id],
          containerIds: [this.newRule.containerId],
        })
        .subscribe((result) => {
    ...
    this.rules
      ...
      .forEach((rec) => rulesId.push(rec.id));
  ...
  showAssignTagsRulesSelected(ruleAssignTagsContent: any) {
    ...
    if (this.validateRulesSelected()) {
      this.apiService.tag.getTags().subscribe((result) => {
        ...
        this.assignTagsToRuleModal = this.modalService.open(
          ruleAssignTagsContent,
          {
            ariaLabelledBy: 'modal-basic-title',
            size: 'lg',
            backdrop: 'static',
  ...
  onAddingTagAssign(tag: any) {
    const existTag = this.tagsSelectedAssign.find((rec) => rec.id === tag.id);
    if (existTag == null) {
  ...
  onRemovingTagAssign(tag: any) {
    this.tagsSelectedAssign = this.tagsSelectedAssign.filter(
      (rec) => rec.id !== tag.id
    );
  }
  ...
  saveAssignTags() {
    ...
    this.tagsSelectedAssign
      .filter((rec) => rec.id.toString() === rec.name)
      .forEach((rec) => (rec.id = -1));
    const rulesId: string[] = [];
    this.rules
      .filter((rec) => rec.selected)
      .forEach((rec) => rulesId.push(rec.id));
    this.apiService.rule
      .assignTagsToRules({ ruleIds: rulesId, tags: this.tagsSelectedAssign })
      .subscribe((result) => {
        this.allRulesSelected = false;
  ...
  validateRulesSelected(): boolean {
    const result =
      ...
      this.rules.filter((rec) => rec.selected).length > 0;
    if (!result) {
      this.toastr.error('Select one rule at least.', 'Error');
    }
    return result;
  }
  ...
  viewRuleExecutionLogDetailsFromOptions(
    ruleExecutionLogDetailRuleEditSearchContent: any,
    ruleChild: Rule
  ) {
            ...
            .subscribe((result) => {
              if (result != null) {
                ...
                this.modalService.open(
                  ruleExecutionLogDetailRuleEditSearchContent,
                  {
                    ariaLabelledBy: 'modal-basic-title',
                    size: 'xl',
                    windowClass: 'modal-custom-xl',
    ...
    this.apiService.ruleExecutionLogDetail
      ...
      .subscribe((result) => {
        if (result != null) {
          ...
          this.toastr.info(
            'No execution log details for rule ' + this.newRule.name,
            'Information'
          );
        }
      });
  ...
  showMoveRulesSelected(moveRuleRuleSearchToContent: any) {
    ...
    if (this.validateRulesSelected()) {
      ...
      this.apiService.container.getChildContainers().subscribe((result) => {
        ...
        this.moveRuleModal = this.modalService.open(
          moveRuleRuleSearchToContent,
          {
            ariaLabelledBy: 'modal-basic-title',
            backdrop: 'static',
          }
