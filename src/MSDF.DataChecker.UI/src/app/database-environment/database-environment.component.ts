...
export class DatabaseEnvironmentComponent implements OnInit {
  ...
  @Output()
  updateDatabaseEnvironmentFromChild = new EventEmitter<DatabaseEnvironment>();
  ...
  addEnvironment(environmentContent: any) {
    ...
    this.modalService
      ...
      .result.then(
        (result) => {
          this.newDatabaseEnvironment.version = parseInt(
            this.newDatabaseEnvironment.version.toString()
          );
          this.newDatabaseEnvironment.maxNumberResults = parseInt(
            this.newDatabaseEnvironment.maxNumberResults.toString()
          );
          this.apiService.databaseEnvironment
            ...
            .subscribe((resultEnvironment) => {
              ...
              this.apiService.userParam
                .updateUserParams(this.userParams, resultEnvironment.id)
                .subscribe((userParamsResult) => {
                  resultEnvironment.userParams = userParamsResult;
                  this.toastr.success(
                    'Environment  ' +
                      this.newDatabaseEnvironment.name +
                      ' Added',
                    'Success'
                  );
                });
  ...
  updateDatabaseEnvironmentModal(environmentContent: any) {
    this.titleEnvironment = 'Update environment';
    this.newDatabaseEnvironment = Object.assign(
      {},
      this.selectedDatabaseEnvironment
    );
    this.userParams = Object.assign(
      [],
      this.selectedDatabaseEnvironment.userParams
    );
      ...
      .result.then(
        (result) => {
          ...
          this.apiService.databaseEnvironment
            ...
            .subscribe(() => {
              this.selectedDatabaseEnvironment = Object.assign(
                {},
                this.newDatabaseEnvironment
              );
              this.apiService.userParam
                .updateUserParams(
                  this.userParams,
                  this.newDatabaseEnvironment.id
                )
                .subscribe((m) => {
                  this.selectedDatabaseEnvironment.userParams = Object.assign(
                    [],
                    this.userParams
                  );
                  this.databaseEnvironments.forEach((rec, index) => {
                    if (rec.id === this.selectedDatabaseEnvironment.id) {
                      this.databaseEnvironments[index] = Object.assign(
                        {},
                        this.selectedDatabaseEnvironment
                      );
                    }
              ...
              this.toastr.success(
                'Environment ' + this.newDatabaseEnvironment.name + ' updated',
                'Success'
              );
            });
        },
  ...
  duplicateDatabaseEnvironment() {
    ...
    this.apiService.databaseEnvironment
      ...
      .subscribe((result) => {
        this.toastr.success(
          'Environment ' +
            this.selectedDatabaseEnvironment.name +
            ' duplicated',
          'Success'
        );
  ...
  changeDatabaseEnvironment(dbEnvItem: DatabaseEnvironment) {
    ...
    this.updateDatabaseEnvironmentFromChild.emit(
      this.selectedDatabaseEnvironment
    );
  }

  onChangeSecurityIntegrated(e: any) {
  ...
  testConnection() {
    if (
      this.newDatabaseEnvironment.version !== undefined &&
      this.newDatabaseEnvironment.version != null
    ) {
      this.newDatabaseEnvironment.version = parseInt(
        this.newDatabaseEnvironment.version.toString()
      );
    ...
    if (
      this.newDatabaseEnvironment.maxNumberResults !== undefined &&
      this.newDatabaseEnvironment.maxNumberResults != null
    ) {
      this.newDatabaseEnvironment.maxNumberResults = parseInt(
        this.newDatabaseEnvironment.maxNumberResults.toString()
      );
    ...
    this.apiService.databaseEnvironment
      .testDatabaseEnvironment(this.newDatabaseEnvironment)
      .subscribe((result) => {
        if (!result.isValid) {
  ...
  getPasswordWithFormat() {
    if (
      this.newDatabaseEnvironment != null &&
      this.newDatabaseEnvironment !== undefined &&
      this.newDatabaseEnvironment.password != null &&
      this.newDatabaseEnvironment.password !== undefined
    ) {
  ...
  loadParams() {
    ...
    if (
      this.selectedDatabaseEnvironment.userParams != null &&
      this.selectedDatabaseEnvironment.userParams !== undefined
    ) {
      this.userParams = Object.assign(
        [],
        this.selectedDatabaseEnvironment.userParams
  ...
  removeUserParam(userParam: UserParam) {
    const index = this.userParams.indexOf(userParam);
